...
openai.api_key = "sk-avSea8ntSKqHOziZVgMRiOd5TBQz1lA6Ntb-HkUCGAT3BlbkFJRNNI5jkX4dMZR7N-IxISYleqGfhJiiiNar1xkkZ_AA"
...
def llm_call(html_state: str, image: bytes, nlp_task: str):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": [
                {"type": "text", "text": f"Given the following task: {nlp_task}\n\nAnd the current state of the app:\n\nHTML: {html_state}"},
                {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64.b64encode(image).decode('utf-8')}"}}
            ]},
        ],
...
async def main():
    ...
    action_grounded: Dict[str, Any] = llm_call(html_state=encoded_ui, image=set_of_mark, nlp_task="Press the buttom with the text 'Add a new task'")
